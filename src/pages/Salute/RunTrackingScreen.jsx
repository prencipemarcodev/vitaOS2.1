...
function RunTrackingScreen({ onFinish, onCancel }) {
  ...
  // Definiamo il contenuto in base allo stato
  let content = null
          ...
          <div className="w-full space-y-3 max-w-xs">
            ...
            <button 
              ...
              className={clsx(
                ...
                permissionStatus === 'denied' ? "bg-gray-200 text-gray-500" : "bg-[var(--color-primary)] text-white shadow-[var(--color-primary-ghost)]"
              )}
              disabled={permissionStatus === 'denied'}
            >
              {permissionStatus === 'denied' ? 'GPS Disabilitato' : 'Consenti accesso GPS'}
            </button>
            <button onClick={onCancel} className="w-full py-4 bg-[var(--bg-elevated)] text-[var(--text-muted)] rounded-2xl font-bold">Non adesso</button>
          </div>
        </div>
      </div>
    )
  ...
  // --- COUNTDOWN ---
    ...
    content = (
      <div className="fixed inset-0 z-[10000] bg-[var(--color-primary)] flex items-center justify-center">
        <motion.span 
          key={countdown}
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          <div className="relative w-32 h-32 mb-8">
            ...
            <motion.div 
              ...
              className={clsx("absolute inset-0 border-4 rounded-full", (accuracy && accuracy < 15) ? "border-t-green-500" : "border-t-[var(--color-primary)]")}
            />
            <div className="absolute inset-0 flex items-center justify-center">
              <motion.div
                animate={{ rotate: [0, -10, 10, -10, 0], y: [0, -5, 0, -5, 0] }}
                transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }}
                className={clsx("transition-colors duration-500", (accuracy && accuracy < 15) ? "text-green-500" : "text-[var(--color-primary)]")}
              >
                <Satellite size={40} />
              </motion.div>
            </div>
          ...
          <Card padding="md" className="w-full max-w-xs grid grid-cols-3 gap-2">
            <div className="text-center border-r border-[var(--border-subtle)]">
              ...
              <p className={clsx("text-xs font-black", (accuracy && accuracy <= 15) ? "text-green-500" : "text-orange-500")}>±{Math.round(accuracy || 0)}m</p>
            </div>
            <div className="text-center border-r border-[var(--border-subtle)]">
              <p className="text-[8px] font-bold text-[var(--text-muted)] uppercase mb-1">Satelliti</p>
              <p className={clsx("text-xs font-black", (accuracy && accuracy < 15) ? "text-green-500" : "text-orange-500")}>{accuracy ? (accuracy < 15 ? 'Ottimo' : 'Fixing...') : '...'}</p>
            </div>
      ...
      <div className="flex flex-col h-full bg-[var(--bg-base)]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
          <AnimatePresence mode="wait">
            {status === 'running' ? (
              <motion.div key="running" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
        ...
        <AnimatePresence>{isFocusMode && <RunFocusMode tracker={tracker} onUnlock={() => setIsFocusMode(false)} />}</AnimatePresence>
      </div>
    )
  }

  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-[var(--bg-base)] flex flex-col overflow-hidden">
      {content}
    </div>,
    document.body
  )
