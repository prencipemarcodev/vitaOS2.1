...
async function saveRun({ tracker, userConfig }) {
  ...
  const newTotal =
    (parseFloat(userConfig.total_run_km_ever) || 0) + tracker.distanceKm;
  await supabase
    .from("user_config")
...
- [x] Migrazione DB eseguita su Supabase
- [x] HTTPS attivo su Vercel (già garantito)
- [x] Permesso GPS testato su iOS Safari + Android Chrome
- [x] WakeLock testato (Android) + fallback messaggio (iOS)
- [x] Mappa carica correttamente (tiles OpenStreetMap)
- [x] Salvataggio corsa su Supabase verificato
- [x] `total_run_km_ever` aggiornato correttamente
- [x] Schermata tracking testata con schermo sempre acceso
- [x] Test su rete lenta (tiles mappa con loading graceful)
- [x] Messaggio "desktop non supportato" su schermi larghi
